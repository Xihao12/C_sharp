    ...
    public partial class PWDForm : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (txtPwd.Text != "" && txtNewPwd.Text != "" && txtCrmPwd.Text != "")
            {
                ...
                //构造SqlCommand对象
                string sql = "select password from bicycle_users where uname='" + Program.userName + "'";
                ...
                if (password == txtPwd.Text)
                {
                    ...
                    {
                        //update
                        cmd.CommandText = string.Format("update bicycle_users set password='{0}' where uname='{1}'", txtNewPwd.Text, Program.userName);//更新sql语句
                        myConn.Open();
